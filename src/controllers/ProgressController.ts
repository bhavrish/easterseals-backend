...
export const saveProgress = async (
  ...
  try {
    ...
    if (response?.rows?.length > 0) {
      return res.status(400).json({
        message:
          "Row already exists, please use the update endpoint to update row",
      });
    }
...
export const updateProgress = async (
  ...
  try {
    // Get the existing progress for this user and course
    const response: QueryResult = await pool.query(
      "SELECT * FROM course_progress WHERE user_id = $1 AND course_id = $2",
      [user_id, course_id]
    );
    ...
    if (response.rows[0].progression == total_pages) {
      await pool.query(
        "UPDATE course_progress SET progression = $1, date_completed = CURRENT_DATE WHERE user_id = $2 AND course_id = $3",
