...
export const getUserProgress = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        const response: QueryResult = await pool.query('SELECT * FROM course_progress WHERE user_id = $1', [user_id]);
...
export const saveProgress = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        await pool.query('INSERT INTO course_progress (progression, total_pages, user_id, course_id) VALUES ($1, $2, $3, $4)', [progression, total_pages, user_id, course_id]);
        return res.json({
...
export const updateProgress = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        await pool.query("UPDATE course_progress SET progression = $1 WHERE user_id = $2 AND course_id = $3",
            [progression, user_id, course_id]);

        return res.json("Progress updated for " + course_id + " course for " + user_id + "user")
    }
    catch (e) {
        console.log(e);
        return res.status(500).json('Internal Server Error');
